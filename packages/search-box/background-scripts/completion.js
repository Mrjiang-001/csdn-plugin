...
class Suggestion {
  ...
  computeRelevancy () {
    ...
    if (this.relevancy == null) { this.relevancy = this.relevancyFunction(this); }
  ...
  generateHtml (request) {
    ...
    let favIcon = "";
    if (this.type === "tab" && !Utils.isFirefox()) { favIcon = `<img class="vimiumReset vomnibarIcon" src="chrome://favicon/size/16@1x/${BgUtils.escapeAttribute(this.url)}" />` }
    if (this.type === "csdn") { favIcon = `<img class="vimiumReset vomnibarIcon" style="width: 17px;" src="/icons/icon-128.png" />` }
    const TYPE_MAP = {
      'bookmark': "书签",
      'history': "历史记录",
      'domain': "域名",
    ...
    if(this.type === 'csdn') {  
      this.html = `\
          <div class="vimiumReset vomnibarTopHalf">
            <span class="vimiumReset vomnibarSource ${insertTextClass}">${insertTextIndicator}</span>
          ...
          <div class="vimiumReset vomnibarBottomHalf vomnibarBottomHalf-csdn">
            <span class="vimiumReset vomnibarSource vomnibarNoInsertText">${insertTextIndicator}</span>
      ...
      this.html = `<div class="vimiumReset vomnibarTopHalf" style="padding-left: 20px;"> 
                      <span class="vimiumReset vomnibarSource vomnibarSource-csdn" style="font-weight:600;">${TYPE_MAP[this.type]}</span>
                      <span class="vimiumReset vomnibarTitle">${this.title}</span>
                    </div>`;  
    } else if(this.type === 'utils'){
      const type2Map = { "calc": '计算结果', "date": '日期', 'json': 'JSON', 'qrcode': '二维码生成' }
      this.html =  `<div class="vimiumReset vomnibarTopHalf">
                      <span class="vimiumReset vomnibarSource ${insertTextClass}">${insertTextIndicator}</span>
                      <span class="vimiumReset vomnibarSource">${type2Map[this.type2]}</span>
                      <span class="vimiumReset vomnibarTitle">${this.type2 === 'date' ? this.description.split('<br>')[0] : this.description}</span> 
                    </div>`
      switch(this.type2){
        case 'json' : this.html = `<div class="vimiumReset vimium-json"> 
                                    <div class="json-result-panel" id="json-format"></div>
                                  </div>`;break; 
                                 
        case 'qrcode' : this.html = `<div class="vimiumReset vimium-qrcode">  
                                        <canvas id="qrcode-panel"></canvas>
                                      </div>`;break;                        
        case 'date' : this.html += `<div class="vimiumReset vomnibarTopHalf" style="padding-left: 66px;margin-top: 5px;"> 
                                    <span class="vimiumReset vomnibarTitle">${ this.description.split('<br>')[1] }</span> 
                                  </div>`;break;                  
      }
  ...
  highlightQueryTerms (string) {
    ...
    for (let term of escapedTerms) { this.pushMatchingRanges(string, term, ranges); }  
  ...
  shortenUrl () {
    ...
    for (let [filter, replacements] of Suggestion.stripPatterns) {
      if (new RegExp(filter).test(url)) {
        for (let replace of replacements) { url = url.replace(replace, ""); } 
      } 
    }
...
class BookmarkCompleter {
  filter ({ queryTerms }, onComplete) {
    if(Settings.excludesMode.indexOf(queryTerms[0]) > -1) { 
      return onComplete([])
    }
  ...
  onBookmarksLoaded () {
    if (this.currentSearch) { this.performSearch(); }
  }
  ...
  performSearch () {
    ...
    if (this.currentSearch.queryTerms.length > 0) {
      results = this.bookmarks.filter(bookmark => {
        ...
        if (bookmark.hasJavascriptPrefix && bookmark.shortUrl == null) { bookmark.shortUrl = "javascript:..."; }
  ...
  traverseBookmarksRecursive (bookmark, results, parent) {
    ...
    if (bookmark.children) { bookmark.children.forEach(child => this.traverseBookmarksRecursive(child, results, bookmark)); }
  }
...
class UtilsCompleter {
  filter ({ queryTerms }, onComplete) { 
    var that = this  
    if (queryTerms.length === 0 || ['ip', 'wd'].indexOf(queryTerms[0]) > -1) {  
      onComplete([]); 
    } else { 
      let result = [] 
      // 计算器
      ...
      // 日期  
      let datestr = queryTerms[0] === 'date' ? queryTerms.slice(1).join(' ')  :   queryTerms.join(' ')  
        ...
        if(dateObj.toString() !== "Invalid Date") {
          ...
          result.push(
            new Suggestion({
              ...
              type: "utils",  
              type2: 'date',
              description: `${dayjsObj.format('YYYY-MM-DD HH:mm:ss')}  时间戳: ${dayjsObj.valueOf()}<br>当前时间: ${dayjs().format('YYYY-MM-DD HH:mm:ss')} 时间戳: ${+new Date()}`,
      ...
      if(queryTerms.length && queryTerms[0] === 'qrcode') {  
        if(queryTerms[0] === 'qrcode' && queryTerms.length === 1) {
          onComplete([]); 
          return
        } 
        result.push(
  ...
  //     if (keyword && ((queryTerms.length > 1) || /\S\s/.test(query))) {
...
class DocCompleter {
  filter ({ queryTerms }, onComplete) {  
    if (queryTerms.length === 0) {  
      ...
      let result = []
      if(queryTerms.length && queryTerms[0] === 'doc') {  
            ...
            result.push(
              new Suggestion({
                ...
                info: {
                  ...
                  css: 'iframe { width: 100%; min-height: 70vh; border:none; }', 
                  loading: true,
                  placeholder: '正在打开文档...'
                },
                autoSelect: true,
...
class FrameCompleter {
  filter ({ queryTerms }, onComplete) { 
    ...
    if (queryTerms.length === 0) {  
      ...
      let result = []   
      if(queryTerms.length && Settings.iframeMap.hasOwnProperty(queryTerms[0])) { 
        let firstQuery = queryTerms[0]
        let info = JSON.parse(JSON.stringify(Settings.iframeMap[firstQuery]))
        let defaultEnableKey = Object.keys(Settings.iframeMap).map(v => Settings.iframeMap[v].defaultEnable ? v : '').filter(v => v)   
        ...
        if(defaultEnableKey.indexOf(firstQuery) > -1) {
          switch(firstQuery){
            case 'ip': suggestionFlag = (!queryword || Utils.checkIP(queryword)); break;
          }
        } else {
          suggestionFlag = !!queryword;   
        }  
        if(suggestionFlag) { 
          info = Object.assign(info, {url: info.url.replace(Settings.iframeReplaceStr, queryword)})  
...
class CSDNCompleter {
  filter ({ queryTerms }, onComplete) {  
    if (queryTerms.length === 0 || (queryTerms.length && Settings.excludesMode.indexOf(queryTerms[0]) > -1)  ) {
      this.csdn = []
      onComplete([]); 
    } else { 
        ...
        continuation: onComplete => {
          $.get('https://so.csdn.net/api/v1/search?platform=pc&s=5&from=chrome_plugin&q=' + encodeURIComponent(queryTerms.join(' ')), res => { 
            if(res.result_vos && res.result_vos.length) { 
              ...
              try {
                onComplete(this.csdn.map((entry, index) => {
                  return new Suggestion({
                    queryTerms,
                    type: "csdn",
                    url: entry.url,
...
class HistoryCompleter {
  filter ({ queryTerms, seenTabToOpenCompletionList }, onComplete) {
    if (((queryTerms.length === 0) && !seenTabToOpenCompletionList) || Settings.excludesMode.indexOf(queryTerms[0]) > -1 ) {
      onComplete([]);
      ...
      HistoryCache.use(history => {
        ...
        onComplete(results.map((entry, index) => {
          return new Suggestion({
            queryTerms,
            type: "history",
            url: entry.url,
...
class DomainCompleter {
  ...
  filter ({ queryTerms, query }, onComplete) {
    ...
    if (this.domains) { this.performSearch(queryTerms, onComplete); } else { this.populateDomains(() => this.performSearch(queryTerms, onComplete)); }
  }
  ...
  onVisitRemoved (toRemove) {
    if (toRemove.allHistory) {
      ...
      toRemove.urls.forEach(url => {
        ...
        if (domain && this.domains[domain] && ((this.domains[domain].referenceCount -= 1) === 0)) { return delete this.domains[domain]; }
      });
    }
  }
...
class TabCompleter {
  filter ({ name, queryTerms }, onComplete) {
    if ((name !== "tabs") && (queryTerms.length === 0)) { return onComplete([]); }
    if (Settings.excludesMode.indexOf(queryTerms[0]) > -1) {
      return onComplete([])
    }
    ...
    chrome.tabs.query({}, tabs => {
      ...
      const suggestions = results.map((tab, index) => {
  ...
  computeRelevancy (suggestion) {
    if (suggestion.queryTerms.length) { return RankingUtils.wordRelevancy(suggestion.queryTerms, suggestion.url, suggestion.title); } else { return BgUtils.tabRecency.recencyScore(suggestion.tabId); }
  }
}
...
class SearchEngineCompleter {
  ...
  filter (request, onComplete) {
    ...
    console.log("CompletionSearch", queryTerms, cachedSuggestions, haveCompletionEngine )
    if ((queryTerms.length === 0) || (cachedSuggestions != null) || !haveCompletionEngine) {
      ...
      if (cachedSuggestions != null) { suggestions.push(...(cachedSuggestions.map(mkSuggestion) || [])); }
...
class MultiCompleter {
  ...
  filter (request, onComplete) {
    ...
    for (let completer of this.completers) {
      if (completer.preprocessRequest) { completer.preprocessRequest(request); } 
    }
    ...
    jobs.onReady(() => {
      let filter; 
      for (filter of filters) { suggestions = filter(suggestions); } 
      ...
      this.filterInProgress = false;
      if (this.mostRecentQuery) { return this.filter(...this.mostRecentQuery); }
    });
  }
  ...
  prepareSuggestions (request, queryTerms, suggestions) {
    ...
    for (let completer of this.completers) {
      if (completer.postProcessSuggestions) { completer.postProcessSuggestions(request, newSuggestions); } 
    } 
    for (let s of newSuggestions) { s.generateHtml(request); }
...
var HistoryCache = {
  ...
  fetchHistory (callback) {
    ...
    return chrome.history.search({ text: "", maxResults: this.size, startTime: 0 }, history => {
      ...
      chrome.history.onVisited.addListener(this.onPageVisited.bind(this));
      chrome.history.onVisitRemoved.addListener(this.onVisitRemoved.bind(this));
      for (callback of this.callbacks) { callback(this.history); }
      this.callbacks = null;
    });
  },
  ...
  onPageVisited (newPage) {
    ...
    if (pageWasFound) { this.history[i] = newPage; } else { this.history.splice(i, 0, newPage); }
  },
