...
class GrabBackFocus extends Mode {
    ...
    Settings.use("grabBackFocus", grabBackFocus => {
      ...
      if (this.modeIsActive) {
        if (grabBackFocus) {
          ...
          if (document.activeElement) { this.grabBackFocus(document.activeElement); }
        } else {
          this.exit();
        }
      }
  ...
  grabBackFocus (element) {
    ...
    if (!this.logged && (element !== document.body)) {
      this.logged = true;
      if (!window.vimiumDomTestsAreRunning) { console.log("An auto-focusing action on this page was blocked by Vimium."); }
    }
    element.blur();
    ...
    if (!request.handler || !!request.name) {
      ...
      if (request.name !== "userIsInteractingWithThePage") {
        if (isEnabledForUrl || (request.name && ["checkEnabledAfterURLChange", "runInTopFrame"].includes(request.name))) { 
          if (request.name) { requestHandlers[request.name](request, sender, sendResponse); } 
        } 
      }
    } 
    return false;
...
var Frame = {
  ...
  init () {
    ...
    return window.addEventListener("unload", (forTrusted(function (event) {
      if (event.target === window) { return disconnect(); }
    })), true);
  },
...
root.lastFocusedInput = (function () {
  ...
  window.addEventListener("focus",
    forTrusted(function (event) {
      ...
      if (DomUtils.isEditable(event.target)) { recentlyFocusedElement = event.target; }
    })
    , true);
