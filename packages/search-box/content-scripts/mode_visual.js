...
class Movement {
  ...
  reverseSelection () {
    ...
    if (element && DomUtils.isEditable(element) && !element.isContentEditable) {
      ...
      for (let i = 0, end = length; i < end; i++) { this.runMovement(this.opposite[direction], character); }
    } else {
      // Normal method.
  ...
  getDirection () {
    ...
    for (let direction of [ forward, backward ]) {
      ...
      if (change = this.extendByOneCharacter(direction)) {
        ...
        if (change > 0) { return direction; } else { return this.opposite[direction]; }
      }
    }
    return forward;
  }
  ...
  collapseSelectionToAnchor () {
    if (this.selection.toString().length > 0) { return this.selection[this.getDirection() === backward ? "collapseToEnd" : "collapseToStart"](); }
  }

  collapseSelectionToFocus () {
    if (this.selection.toString().length > 0) { return this.selection[this.getDirection() === forward ? "collapseToEnd" : "collapseToStart"](); }
  }
  ...
  selectLexicalEntity (entity, count) {
    ...
    if (entity === word) { this.runMovement([ forward, character ]); }
    this.runMovement([ backward, entity ]);
    this.collapseSelectionToFocus();
    for (let i = 0, end = count; i < end; i++) { this.runMovement([ forward, entity ]); }
  }
...
VisualMode.prototype.movements = {
  ...
  "c" () {
    ...
    if ((this.name === "caret") || (this.selection.toString().length <= 1)) { this.movement.collapseSelectionToAnchor(); } else { this.movement.collapseSelectionToFocus(); }
      ...
      case "string":
        for (let i = 0, end = count; i < end; i++) {
          ...
          if (this.selection.isCollapsed) {
            ...
            if ((this.movement.getDirection() !== direction) && (granularity === "line")) { this.movement.reverseSelection(); }
            this.movement.runMovement(command);
          }
        }
        break;
