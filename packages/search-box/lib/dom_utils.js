...
var DomUtils = {
  ...
  getVisibleClientRect (element, testChildren) {
    ...
    const clientRects = ((() => {
      const result = [];
      for (clientRect of element.getClientRects()) { result.push(Rect.copy(clientRect)); }
      return result;
    })());
        ...
        for (let child of Array.from(element.children)) {
          ...
          if ((computedStyle.getPropertyValue("float") === "none") &&
              ...
              !((clientRect.height === 0) && isInlineZeroHeight() &&
                (computedStyle.getPropertyValue("display").indexOf("inline") === 0))) {
            continue;
          }
  ...
  simulateClickDefaultAction (element, modifiers) {
    ...
    if (newTabModifier) {
      ...
      chrome.runtime.sendMessage({handler: "openUrlInNewTab",
        url: element.href,
        active:
        shiftKey === true});
